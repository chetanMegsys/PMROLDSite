...
<head>
    <meta charset="utf-8">
    <meta charset="utf-8">
    <title>Purchase Report Via PayPal - <?php echo $report_data[0]['rep_name']; ?></title>
    <meta name="description" content="Purchase report with PayPal - <?php echo $report_data[0]['rep_name']; ?>">
    ...
    "purchase_units" => array($pu)
	  ...
	  paypal.Buttons({
        ...
        createOrder: (data, actions) => {
          return fetch("<?php echo WEBSITE_URL ; ?>checkout/finalOrder", {
            method: "post",
        ...
        onApprove: (data, actions) => {  
          return fetch('<?php echo WEBSITE_URL ; ?>checkout/CapturePaypalOrder2', {
            method: "post",
          })
          .then((response) => response.json())
          .then((orderData) => {
			...
			if(transaction.status == 'COMPLETED' ){
				  window.location = '<?php echo WEBSITE_URL ; ?>checkout/thankyou';
			}
			else{
