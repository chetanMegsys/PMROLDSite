...
<head>
    ...
    <style>.media-citations a{color: #2358a8 !important;} img.media-citation-logo {width: 100%;padding: 10px;} .aboutListSec p{font-size:18px;} .secHeading{margin:20px 0px; color:#000; font-weight:600;}.p-title{font-size:2rem;font-weight:600;}
    html {
    ...
    <script>
(function () {
  // Selectors: adjust if your header selector differs
  var tocLinks = document.querySelectorAll('.toc a[href^="#"]');
  var header = document.querySelector('header'); // used to compute offset
  ...
  function smoothScrollTo(targetEl) {
    var targetTop = getOffsetTop(targetEl);
    var scrollTo = Math.max(0, targetTop - headerHeight - 12); // 12px breathing room
    // Use window.scrollTo with smooth behavior for consistent result
    window.scrollTo({
      top: scrollTo,
      behavior: 'smooth'
    });
  ...
  tocLinks.forEach(function(link) {
    link.addEventListener('click', function(e) {
      // allow default if it's an external link (safety)
      if (!link.hash) return;
  ...
  // Optional: if user loads page with a hash, adjust scroll so header doesn't hide it
  window.addEventListener('load', function() {
    if (location.hash) {
      var id = decodeURIComponent(location.hash.slice(1));
      var target = document.getElementById(id);
      if (target) {
        // small timeout to let browser finish native jump before our correction
        setTimeout(function(){ smoothScrollTo(target); }, 10);
      }
    }
  }, false);
})();
