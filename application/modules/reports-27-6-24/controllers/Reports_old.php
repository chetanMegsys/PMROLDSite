<?php
    ...
    class Reports extends MY_Controller{
        ...
        public function market_research_list(){
           // echo "https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
            //echo $url = $offer_url = current_url();
            //$hash = "";
            /**?>
            <script>
           ...
           // echo $endofurl = substr($r['path'], strrpos($r['path'], '#'));
        ...
        public function report_detail($report_url){
            ...
            if (isset($_POST['btnSubmit']) && !$this->is_bot($_SERVER['HTTP_USER_AGENT']) ) { 
                ...
                if($rsContact){
                    ...
                    $email_setting['subject'] = "Persistence Market Research ".$email_setting['source']." Enquiry";
        ...
        public function report_detail_amp($report_url){
            ...
            if (isset($_POST['btnSubmit']) && isset($_POST['captcha']) && $_POST['captcha'] != "") {
                ...
                unset($_POST['btnSubmit'],$_POST['captcha'],$_POST['countryCode']); 
                $postData = $_POST;
    ...
    public function cta_mobile()
    {
          $data['report_detail'] = $this->reports_model->getReportDetailByURL("emission-control-systems-market");
         $detect = new Mobile_Detect();
                if ($detect->isMobile() || $detect->isTablet() || $detect->isAndroidOS()) {
                    $this->load->view("cta_popup_mobile",$data);
                }else{
                    $this->load->view("cta_popup_web",$data);
                }
    }

    public function cta_web()
    {
         $data['report_detail'] = $this->reports_model->getReportDetailByURL("emission-control-systems-market");
          $detect = new Mobile_Detect();
                if ($detect->isMobile() || $detect->isTablet() || $detect->isAndroidOS()) {
