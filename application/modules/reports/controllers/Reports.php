<?php
    ...
    class Reports extends MY_Controller{
        ...
        public function market_research_list(){
            ...
            $page = $this->input->get('page'); // get ?page= value
            // If page parameter exists and is numeric, redirect to main page
            if (!empty($page) && is_numeric($page)) {
                redirect('market-research.asp', 'location', 301);
            }
            ...
            $data['categorytree'] = $this->reports_model->getCategoryTree();
            ...
            $data['actual_canonical_url'] = WEBSITE_URL."market-research.asp";
        ...
        public function market_research_upcoming_list(){

            $page = $this->input->get('page'); // get ?page= value
            // If page parameter exists and is numeric, redirect to main page
            if (!empty($page) && is_numeric($page)) {
                redirect('forthcoming-reports.asp', 'location', 301);
            }
           ...
           // }
            ...
            $data['reports'] = $this->reports_model->getUpcomingReports($num_rec_per_page, $start_from, $filter);
            ...
            $data['meta_description'] = "Persistence Market Research - PMR provides businesses to shape themselves by giving 360-degree analysis, better customer satisfaction with laser edge precision & quality";
            $data['meta_title'] = "Explore Benefits Of Crisp & Conclusive Market Research Reports | Persistence Market Research (PMR)";
            $data['page_heading'] = "Upcoming Market Research Reports";
            ...
            $data['breadcrumb'][0]['title'] = "Upcoming Market Research Reports";
            $data['actual_canonical_url'] = WEBSITE_URL."forthcoming-reports.asp";
            $detect = new Mobile_Detect();
            if ($detect->isMobile() || $detect->isTablet() || $detect->isAndroidOS()) {
                $this->load->view("market_research_upcoming_list_mobile",$data);
            }else{
                $this->load->view("market_research_upcoming_list",$data);
            }
        }
        ...
        public function upcoming_report_list(){  
           ...
           // redirect(WEBSITE_URL.'market-research.asp#forthcoming-reports',"301");
            redirect(WEBSITE_URL."market-research.asp#forthcoming-reports",'location',301);
        ...
        public function category_reports($category_url){
           ...
           // exit();
            ...
            if(!empty($categoryDetails[0]['parent_id'])){
                $data['parent_category'] = $this->reports_model->getReportParentCat($categoryDetails[0]['parent_id']);
            } else {
                $data['parent_category'] = $this->reports_model->getReportParentCat($categoryDetails[0]['category_id']);
            }
            
            if(NULL == $categoryDetails[0]['parent_id']){
                $data['reports'] = $this->reports_model->getParentCategoryReports($num_rec_per_page, $start_from, $filter, $categoryDetails[0]['category_id']);
                $data['total_records'] = $this->reports_model->getParentCategoryReportCount($filter, $categoryDetails[0]['category_id']);
                //var_dump($data['total_records']);
            }else {
           ...
           // var_dump($data['reports']);
            ...
            $data['page_heading'] = $categoryDetails[0]['cat_name'];

            $data['hdn_cat_id'] = $categoryDetails[0]['category_id'];

            $data['meta'] = array('viewport','X-UA-Compatible','dc-description');
            $data['actual_canonical_url'] = WEBSITE_URL."market-research-report/".$category_url.".asp";
            if(isset($data['parent_category'][0]['seo_pagename']) && $category_url !== $data['parent_category'][0]['seo_pagename'] ){
                $data['breadcrumb'][0]['link'] = WEBSITE_URL."market-research-report/".$data['parent_category'][0]['seo_pagename'].".asp";
                $data['breadcrumb'][1]['link'] = WEBSITE_URL."market-research-report/".$category_url.".asp";
                $data['breadcrumb'][0]['title'] = $data['parent_category'][0]['cat_name'];
                $data['breadcrumb'][1]['title'] = ucwords(str_replace("-"," ",$category_url));
             } else {
                $data['breadcrumb'][0]['link'] = WEBSITE_URL."market-research-report/".$category_url.".asp";
                $data['breadcrumb'][0]['title'] = ucwords(str_replace("-"," ",$category_url));
             }
            
           

           
           //var_dump($data['parent_category'][0]['seo_pagename']);exit;
           // $data['reports'] = $this->reports_model->getParentCategoryReports($num_rec_per_page, $start_from, $filter, $data['parent_category']->seo_pagename);
            // var_dump($data['hdn_cat_id']);
            if (isset($_POST['page'])){
                $detect = new Mobile_Detect();
                if ($detect->isMobile() || $detect->isTablet() || $detect->isAndroidOS()) {
        ...
        public function report_detail($report_url){
           ...
           // $data['report_detail'] = $this->reports_model->getReportDetailByURL($report_url);    
            $data['report_detail'] = $this->reports_model->getReportDetailByURLByIsIndiexAuthor($report_url);             
            ...
            if (isset($_POST['btnSubmit']) && !$this->is_bot($_SERVER['HTTP_USER_AGENT']) ) { 
                ...
                if($rsContact){
                    ...
                    if($segment == "S"){
                        ...
                        $email_setting['source'] = "Request Report Sample (POP)";
                        ...
                        $url = WEBSITE_URL . 'thanks/enquiry/'.$id;
                        $email_setting['source'] = "Promotional Offer";
                    }
            ...
            if(!empty($data['report_detail'])){
                ...
                //var_dump($categoryDetails[0]['parent_id']);
                $data['parent_category']=$this->reports_model->getReportParentCat($data['report_detail'][0]['parent_id']);

                if(!empty($data['parent_category'])){
                    $data['parent_category'] = $this->reports_model->getReportParentCat($data['report_detail'][0]['parent_id']);
                } else {
                    $data['parent_category'] = $this->reports_model->getReportParentCat($data['report_detail'][0]['category_id']);
                }
               
                $data['get_authors'] = $this->author_model->getAuthorById($data['report_detail'][0]['author_id']);
               // var_dump ($data['get_authors']);
                ...
                elseif($data['report_detail'][0]['rep_status'] == "Y"  && $data['report_detail'][0]['status'] == "A"  )
                {
                    ...
                    $data['meta_dc_desc'] = isset($data['report_detail'][0]['dc_desc']) ? $data['report_detail'][0]['dc_desc'] : $data['report_detail'][0]['meta_title'];

                    if($data['report_detail'][0]['is_index'] == 'I')
                    {
                    $data['meta'] = array('report_detail','viewport', 'index');
                    }
                    ...
                    $data['related_reports_key'] = $this->reports_model->getRelatedReport_byCat($data['report_detail'][0]['rep_id'], $data['report_detail'][0]['rep_keyword'], $data['report_detail'][0]['cat_id']);
                    $data['faqs'] = $this->reports_model->getFaqs($data['report_detail'][0]['rep_id']);

                    // $data['breadcrumb'][0]['link'] = WEBSITE_URL."market-research.asp";
                    // $data['breadcrumb'][0]['title'] = "Market Research";

                    $data['breadcrumb'][0]['link'] = WEBSITE_URL . 'market-research-report/' . $data['report_detail'][0]['seo_pagename'].'.asp';
                    $data['breadcrumb'][0]['title'] = $data['report_detail'][0]['cat_name'];

                    $data['breadcrumb'][1]['link'] = WEBSITE_URL."market-research/".$data['report_detail'][0]['rep_url'].'.asp';
        ...
        public function report_toc($report_url){
            ...
            if(!empty($data['report_detail'])){
                ...
                //var_dump($data['report_detail'][0]['parent_id']);
                if(!empty($data['report_detail'][0]['parent_id'])){
                    $data['parent_category'] = $this->reports_model->getReportParentCat($data['report_detail'][0]['parent_id']);
                } else {
                    $data['parent_category'] = $this->reports_model->getReportParentCat($data['report_detail'][0]['category_id']);
                }
        ...
        public function report_rm($report_url){
            ...
            if(!empty($data['report_detail'])){
                ...
                $detect = new Mobile_Detect();
                if ($detect->isMobile() || $detect->isTablet() || $detect->isAndroidOS()) {
                    $this->load->view("report_details_mobile_research_methodology",$data);
                }else{
                    $this->load->view("report_details_research_methodology",$data);
                }
        ...
        public function getReportsHTML($reports){
            ...
            if(!empty($reports)){
                foreach($reports as $report){
                    ...
                    if( 0 >  $report['rep_pages']){
                        $page_no_data = ' | Pages: ' . $report['rep_pages'];
                    } else {
                        $page_no_data = '';
                    }
                    ...
                    $html .= '<div class="row reportListing">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                    <div class="contentBox py-0">
                                        ...
                                        <p class="my-1">'.$report['rep_name'].'</p>
                                        <span class="report-meta">' . date("F, Y", strtotime($report['update_date']))  . $page_no_data . ' | Format: PPT*, PDF, EXCEL
                                            </span>
                                        <a class="rmlink" href="'. WEBSITE_URL.'market-research/'.$report['rep_url'].'.asp" title="'. $at_keyword .'">Read More →</a>
                                    </div>
                                </div>
                            </div>';
                }
        ...
        public function category_redirect($slug = ''){

             // Trim whitespace just in case
            $slug = trim($slug);

            // Map specific slugs to new ones
            $slug_map = [
                'automotive-and-transportation' => 'automotive',
                'ict' => 'it-and-telecommunications',
                'electronics-and-smart-devices' => 'semiconductor-electronics'
            ];

            // If slug exists in the map, replace it
            if (array_key_exists($slug, $slug_map)) {
                $slug = $slug_map[$slug];
            }
             $new_url = base_url(WEBSITE_URL.'market-research-report/' . $slug . '.asp');
             redirect(WEBSITE_URL.'market-research-report/' . $slug . '.asp','location',301);
        }
    ...
    public function getReportListingData()
    {
       /// print_r($_GET);
        if (!$this->input->is_ajax_request()) {
            show_404(); // or show_error('No direct script access allowed', 403);
            return;
        }
        $page = $_GET['page'];
        ...
        if(isset($_GET['catid']) && $_GET['catid'] != '' ){
            $categoryIDs = $this->reports_model->getChildCategories($_GET['catid']);
            if(!empty($categoryIDs)){
                $filter['cat_id'] = $categoryIDs;
            }else {
                $filter['cat_id'] =  $_GET['catid'];
            }
        }
         $repType = '';
      ...
      if( 'M' == $repType){
         $reports = $this->reports_model->getUpcomingReports($num_rec_per_page,$start_from,$filter);
      } else {
         $reports = $this->reports_model->getReports($num_rec_per_page,$start_from,$filter);
      }
        ...
        if(count($reports) > 0)
        {
          foreach($reports as $report){
                    ...
                    if(0 !==  $report['rep_pages'] ){
                        ...
                        $page_no_data = 'Report Type: Upcoming';
                    }
                   // var_dump($page_no_data);exit();
                    if('Pages: 0' == $page_no_data){
                      ...
                      $html .= '<div class="row reportListing">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                    <div class="contentBox py-0">
                                        <h2 class="mt-0"><a class="listTitle secHeadingLeft" href="'.WEBSITE_URL."market-research/".$report['rep_url'].".asp".'" title="'.$keyword.'">'.$keyword.'</a> </h2>
                                        <p class="my-1">'.$report['rep_name'].'</p>
                                        <span class="report-meta">' . date("j M Y", strtotime($report['update_date'])) . ' | Format: PPT*, PDF, EXCEL
                                            </span>
                                        <a class="rmlink" href="'. WEBSITE_URL.'market-research/'.$report['rep_url'].'.asp" title="'. $at_keyword .'">Read More →</a>
                                    </div>
                                </div>
                        ...
                        $html .= '<div class="row reportListing">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-12">
                                    <div class="contentBox py-0">
                                        <h2 class="mt-0"><a class="listTitle secHeadingLeft" href="'.WEBSITE_URL."market-research/".$report['rep_url'].".asp".'" title="'.$keyword.'">'.$keyword.'</a> </h2>
                                        <p class="my-1">'.$report['rep_name'].'</p>
                                        <span class="report-meta">' . date("j M Y", strtotime($report['update_date'])) . ' | ' . $page_no_data . ' | Format: PPT*, PDF, EXCEL
                                            </span>
                                        <a class="rmlink" href="'. WEBSITE_URL.'market-research/'.$report['rep_url'].'.asp" title="'. $at_keyword .'">Read More →</a>
                                    </div>
                                </div>
    ...
    public function getChildReportListingData()
    {
       /// print_r($_GET);
        if (!$this->input->is_ajax_request()) {
            show_404(); // or show_error('No direct script access allowed', 403);
        ...
        if(isset($_GET['searchBySector']) && $_GET['searchBySector'] != '' ){
            $categoryIDs = $this->reports_model->getChildCategories($_GET['searchBySector']);
            if(!empty($categoryIDs)){
                $filter['cat_id'] = $categoryIDs;
            }else {
                $filter['cat_id'] =  $_GET['searchBySector'];
       ...
       // print_r(getReportParentCat($filter['cat_id']));
        //exit();
         $repType = '';
        if(isset($_GET['searchByType']) && $_GET['searchByType']!=''){
            $filter['reportType']  = $_GET['searchByType'];
