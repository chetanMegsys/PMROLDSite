    ...
    class Sample extends MY_Controller{
        ...
        public function index($id){
	        ...
	        if($segment == "samples"){
               ...
               $data['pageHeadingnew'] = 'Get <span class="bold500"><u>FREE</u></span> Report Sample';
                $data['ButtonHeading'] = 'Yes! Send a <span class="bold700"><u>FREE</u></span> Copy';
	            $data['enqType'] = "S";
                $email_setting['source'] = "Get Free Report Sample";
	        }else if($segment == "methodology"){
	            $data['pageHeading'] = 'Request Report Methodology';
                 $data['ButtonHeading'] = 'Get Report Methodology';
	            $data['enqType'] = "RRM";
                $email_setting['source'] = "Request Report Methodology";
	        }else if($segment == "request-advisory"){
	            $data['pageHeading'] = 'Request Advisory';
                $data['ButtonHeading'] = 'Connect With Advisory';
	            $data['enqType'] = "RA";
        	...
        	if (isset($_POST['btnSubmit']) && !$this->is_bot($_SERVER['HTTP_USER_AGENT']) ){ //echo "Wait"; exit();
               $_POST  = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
                ...
                $postData['jobTitle'] = $this->data_filter_post($_POST['jobTitle']);
                $postData['company'] = $this->data_filter_post($_POST['company']);
