<?php 
    ...
    class Sitemap extends MY_Controller{
        ...
        public function html_sitemap(){
            ...
            $data['actual_canonical_url'] = WEBSITE_URL."sitemap.html";
          ...
          //  $sitemap = '<!-- Total links:' . $total_results .' -->';
            ...
            foreach($result as $value)
            {
                ...
                if ($added_date && strtotime($added_date)) {
                   ...
                   $formatted_date = date('Y-m-d', strtotime($added_date));
                } else {
        ...
        public function xml_sitemap_pages(){
            ...
            $static_urls = [
                ...
                WEBSITE_URL . "services/syndicate-market-research.asp",
                WEBSITE_URL . "services/custom-research.asp",
                WEBSITE_URL . "services/consulting-services.asp",
                WEBSITE_URL . "services/full-time-engagement.asp",
                WEBSITE_URL . "services/primary-research-capabilties.asp",
                WEBSITE_URL . "services/competitive-intelligence.asp",
                WEBSITE_URL . "services/commercial-due-diligence.asp",
        ...
        public function xml_sitemap_23_7_25(){
           ...
           // $sitemap .= "</sitemap>";
            ...
            // Industries Sitemap
            $sitemap .= "<sitemap>";
            $sitemap .= "<loc>" . WEBSITE_URL . "sitemap-industries.xml</loc>";
            $sitemap .= "</sitemap>";
        ...
        public function sitemap_industries(){
            $sitemap = "<urlset xmlns='http://www.sitemaps.org/schemas/sitemap/0.9' xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xsi:schemaLocation='http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd'>";

            $categories = $this->sitemap_model->getCategories();
            if(!empty($categories)){
                $i=0;
                foreach($categories as $cat){
					 $date_modified = $cat->date_modified != '0000-00-00 00:00:00' ? $cat->date_modified : date('Y-m-d');
                     if ($date_modified && strtotime($date_modified)) {
                        $formatted_date = date('Y-m-d\TH:i:sP', strtotime($date_modified)); // Correct format for Google sitemap
                    } else {
                        $formatted_date = date('Y-m-d\TH:i:sP'); //Fallback to the current date/time
                    }
                     //  $sitemap = '<!-- Total links:' . $cat->seo_pagename .' -->';
                 //    if($i<20) {
                        $sitemap .= "<url><loc>".strtolower(WEBSITE_URL."market-research-report/".url_title($cat->seo_pagename, 'dash', true)).".asp</loc> <changefreq>daily</changefreq><priority>0.9</priority></url>";
                    //    $i = $i +1;
                   // } else {
                     //  $sitemap .= "<url><loc>".WEBSITE_URL . "market-research-report/" . url_title($cat->seo_pagename, 'dash', true).".asp</loc> <lastmod>" . $formatted_date . "</lastmod><priority>0.9</priority></url>";
                   // }
                }
            }
