<?php
    ...
    class Reports_Model extends CI_Model{
        ...
        public function getReports($num_rec_per_page, $start_from, $filter, $reportType=""){
            ...
            if(isset($filter['rep_pub_date']) && $filter['rep_pub_date'] != ""){
               // $this->db->where("YEAR(rep_pub_date)",$filter['rep_pub_date']);
                $this->db->where("YEAR(update_date)",$filter['rep_pub_date']);
            }

            if($reportType != ""){
                $this->db->where("rep_type",$reportType);
            }
          ...
          //  if($reportType=='N'){
           ...
           // }
        	...
        	return $this->db->where("status","A")->where("rep_status",'Y')->where("is_custom",'N')->order_by($field,"desc")->limit($num_rec_per_page, $start_from)->get()->result_array();
        }

        public function getReportCount($filter,$reportType=""){
        	...
        	return $this->db->where("status","A")->where("rep_status",'Y')->where("is_custom",'N')->get()->num_rows();
        }
        ...
        public function getReportDetailByURL($report_url){
            return $this->db->select("reports.is_index,reports.rep_id, reports.rep_name, reports.rep_keyword, reports.rep_price_sul,reports.rep_tag_id,reports.rep_breadcrumb, reports.rep_url, reports.rep_pages, reports.update_date as rep_pub_date, reports.company, reports.rep_desc, reports.rep_type, reports.meta_title, reports.meta_keywords, reports.meta_desc, reports.dc_desc, category.cat_name, category.category_id, category.seo_pagename")->from("reports")->join("category","category.category_id=reports.cat_id")->where("reports.rep_url",$report_url)->where("reports.rep_status","Y")->where("reports.status","A")->where("reports.is_custom",'N')->get()->result_array();
        }
        ...
        public function getRedirections($url,$type=""){
			$this->db->select("destination_url,error_type")->from("pageredirections");
            if($type!="")
                $this->db->where("old_url",$url);
